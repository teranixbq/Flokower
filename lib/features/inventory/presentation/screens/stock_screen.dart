      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          if (_tabController.index == 0) {
            showDialog(context: context, builder: (ctx) => const MaterialForm()).then((result) {
              if (result == true && mounted) {
                showToast(context, message: 'Bahan berhasil ditambahkan!', type: ToastType.success);
              }
            });
          } else {
            showDialog(context: context, builder: (ctx) => const ProductFormDialog()).then((result) {
              if (result == true && mounted) {
                showToast(context, message: 'Produk berhasil ditambahkan!', type: ToastType.success);
              }
            });
          }
        },
